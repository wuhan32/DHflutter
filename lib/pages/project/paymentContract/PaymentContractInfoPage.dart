...
class PaymentContractInfoPage extends StatefulWidget {
  final String Id;

  PaymentContractInfoPage(this.Id) : super();

  @override
  _PaymentContractInfoPageState createState() =>
      _PaymentContractInfoPageState();
}

class _PaymentContractInfoPageState
    extends State<PaymentContractInfoPage> {
  var Info={};
  @override
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: new ListView(children: <Widget>[
          ...
          buildInfoItem("付款编号", Info["paymentcode"]== null ? "" : Info["paymentcode"]),
          Divider(
            height: 1.0,
            color: Colors.black12,
          ),
          buildInfoItem("付款名称", Info["paymentname"]== null ? "" : Info["paymentname"]),
          Divider(
            height: 1.0,
            color: Colors.black12,
          ),
          ...
          buildInfoItem("付款方式", Info["paymenttypename"]== null ? "" : Info["paymenttypename"]),
          Divider(
            height: 1.0,
            color: Colors.black12,
          ),
  ...
  Future<Map> toInfo() async {
    Response results = await HttpUtil.getInstance(context)
        .post("paymentContract/appFormLoad", data: {
      "id": widget.Id,
    });
    print("id:" + widget.Id);
